...
int analyze_signal(void* arg,signal *sig, int filter_order, int num_bands, double *lb, double *ub)
{
    double Fc, bandwidth;
...
long myid = (long)arg;
int j;
int blocksize=num_bands / num_threads; //should it be numbands or bandwidth?
...
#define ALIENS_HIGH  150000.0
    ...
    *lb=*ub=-1;
    ...
    for (band=mystart;band<myend;band++) {//edited 
...
int main(int argc, char *argv[])
{
    ...
    int num_bands;
 //   int num_threads;
   // int num_procs; //new parallel args
    double start, end;
 ...
 for (i=0;i<num_threads;i++) {
     struct args myargs; //need to initialize, ask Shae, also do I want to make a new struct for each thread?
     myargs.idnum = i;  //also need to change analyze signal to accept args this way 
     myargs.sig = sig;
        myargs->lb = start;
        myargs->ub = end;
        myargs.filter_order = filter_order;
        myargs.num_threads = num_threads;
        myargs.num_procs = num_procs// double check these, probably wrong or incomplete
        rc = pthread_create( &(tid[i]),
                NULL,
                analyze_signal,
               (void*) i);//, sig, filter_order, num_bands, &start, &end); //pass arguments?
        if (rc!=0) {
...
//   if (analyze_signal((void*) i, sig,filter_order,num_bands,&start,&end)) { 
